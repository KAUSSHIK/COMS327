...
{
  static int next = 0;
  static char s[20][12]; // Array of 20 strings, each with space for 11 characters + null terminator.
  ...
  if (i == INT_MAX) {
    s[next][0] = '\0'; // If i is INT_MAX, return an empty string.
  } else {
    // Safely print the integer to the string buffer, ensuring no overflow.
    snprintf(s[next], sizeof(s[next]), "%d", i);
  }

  return s[next++]; // Return the current string and increment the index for the next call.
}
...
void db_parse(const char *file_name, bool print)
{
  FILE *f;
  char line[800];
  int i;
  ...
  if (print && strcmp(file_name, "pokemon") == 0) {
    for (i = 1; i < 1093; i++) {
  ...
  if (print && strcmp(file_name, "moves") == 0) {
    for (i = 1; i < 845; i++) {
  ...
  if (print && strcmp(file_name, "pokemon_moves") == 0) {
    for (i = 1; i < 528239; i++) {
        printf("%s,%s,%s,%s,%s,%s\n",
  ...
  if (print && strcmp(file_name, "pokemon_species") == 0) {
    for (i = 1; i < 899; i++) {
 ...
 if (print && strcmp(file_name, "experience") == 0) {
    for (i = 1; i < 601; i++) {
        printf("%s,%s,%s\n",
  ...
  if (print && strcmp(file_name, "type_names") == 0) {
    for (i = 1; i < 19; i++) {
        printf("%s\n", types[i]);
    }
}
  ...
  if (print && strcmp(file_name, "pokemon_stats") == 0) {
    for (i = 1; i < 6553; i++) {
        printf("%s,%s,%s,%s\n",
  ...
  if (print && strcmp(file_name, "stats") == 0) {
    for (i = 1; i < 9; i++) {
        printf("%s,%s,%s,%s,%s\n",
               i2s(stats[i].id),
  ...
  if (print && strcmp(file_name, "pokemon_types") == 0) {
    for (i = 1; i < 1676; i++) {
        printf("%s,%s,%s\n",
