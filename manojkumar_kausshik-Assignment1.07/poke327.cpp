...
int main(int argc, char *argv[])
 {
  ...
  if (argc < 2) {
    fprintf(stderr, "Usage: %s <file_name_without_csv>\n", argv[0]);
    return 1;
  }

  char* filename = argv[1]; // assuming the file name is the first argument.
  db_parse(filename, true); // Now db_parse takes the file name as an argument.
